   ...
   <div class="column content" color=white
    ...
    <!--<form action="/action_page.php">-->
       <?php
		...
		echo"<label for=\"actorname\">Search Actor:</label>
   ...
   if (!mysql_select_db("CS143", $db_connection)){
       ...
       exit(1);
		 ...
		 if (count($search)==1)
		 {
			 ...
			 $query = "SELECT id,last,first,dob FROM Actor WHERE first='$search[0]' OR last='$search[0]'";
			 $query = "SELECT id, last, first, dob FROM Actor AS CNAME WHERE CNAME.first LIKE '%$search[0]%' OR CNAME.last LIKE '%$search[0]%'";
		 }
		 else if (count($search) == 2)
		 {
			 $query = "SELECT id,last,first, dob FROM Actor WHERE first='$search[0]' AND last='$search[1]'";
			 $query = "SELECT id,last, first, dob FROM Actor AS CNAME WHERE (CNAME.first LIKE '%$search[0]%' AND CNAME.last LIKE '%$search[1]%')
    ...
    echo "<div class=\"footer\">
	 ...
	 {
		 echo "<div class=\"failedanswer\">";
    echo "Error: must type in an actor's name and a movie's title : Refreshing Page";
    echo "</div>";
    echo "<div class=\"footer\">
          <p>By Avery Wong</p>
          </div>"; 
	   ...
	   if (isset($_POST['submit'])&& $_SERVER["REQUEST_METHOD"] == "POST" ) {
	     echo"<form method=\"post\" action=" .htmlspecialchars($_SERVER["PHP_SELF"]). ">  ";
	     echo "<label for=\"movie title\">Pick a Movie the Actor/Actress acted in:</label>";
          $db_connection = mysql_connect("localhost", "cs143", "");
        if (!$db_connection) {
          $errormsg = mysql_error($db_connection);
          echo "Error connecting to MySQL: " . $errormsg . "<br/>";
...
$insert= "";
 ...
 if (isset($_POST['continue'])&& $_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $mid = intval($mid);
  $name = intval($name);
  $insert = "INSERT INTO MovieActor VALUES($mid, $name, \"$role\")";
